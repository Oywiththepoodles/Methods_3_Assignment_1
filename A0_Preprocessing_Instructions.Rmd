...
#We change all participants ID to "ID", all visits to "visit"
Demo <- Demo %>% 
  rename(ID = Child.ID)
...
```{r}
#homogenizing the visit, so it is only a number
LU <- LU %>% 
...
```{r}
Demo <- Demo %>% 
  mutate(ID = gsub('\\.','',ID)) # inside the '' is what we want to remove, the punktum, and inside the next '' is what we want to replace it with, nothing
...
Subset_demo <- Subset_demo %>% 
  rename(
    Autism_severity = ADOS,
    Non_verbal_iq = MullenRaw,
...
Subset_token <- Subset_token %>% 
  rename(
    Unique_words_mom = types_MOT,
    Unique_words_child = types_CHI,
    Sum_of_words_mom = tokens_MOT,
    Sum_of_words_child = tokens_CHI)
```
...
```{r}
#Getting visit to be the right factor:
Subset_token$Visit<-as.numeric(Subset_token$Visit)
Subset_token$Visit<-as.factor(Subset_token$Visit)
class(Subset_token$Visit)

Subset_LU$Visit<-as.numeric(Subset_LU$Visit)
Subset_LU$Visit<-as.factor(Subset_LU$Visit)
class(Subset_LU$Visit)

Subset_demo$Visit<-as.factor(Subset_demo$Visit)
class(Subset_demo$Visit)

#Merging the datasets into one:
Analysis_df <- Subset_demo %>% 
  full_join(Subset_LU) %>% 
  full_join(Subset_token)
```
...
sub_1 <- Analysis_df %>% 
  ...
  filter(Visit == 1) %>% 
  rename(
    Autism_severity_1 = Autism_severity,
    Non_verbal_iq_1 = Non_verbal_iq,
    Verbal_iq_1 = Verbal_iq,
    Socialization_1 = Socialization) %>% 
  select(-Visit)

clinical_df <- Analysis_df %>% 
  full_join(sub_1)

### why do we keep the original autism severity
```
...
clinical_df$ID <- clinical_df$ID %>% as.factor() #getting it from character to factor
clinical_df$ID <- clinical_df$ID %>% as.numeric() #making to names/initials into numbers
clinical_df$ID <- clinical_df$ID %>% as.factor()

#Renaming variable values
clinical_df$Gender <- clinical_df$Gender %>% as.factor()
levels(clinical_df$Gender) <- c("Female", "Male")

clinical_df$Diagnosis <- clinical_df$Diagnosis %>% as.factor()
levels(clinical_df$Diagnosis) <- c("ASD", "TD")

#Saving the dataset into a csv file
write_csv(clinical_df,"clinical_data.csv")
